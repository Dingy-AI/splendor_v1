...
class SplendorEnv(gym.Env):
    ...
    def slow_3_generate_valid_payments(
        self,
        player: Player,
        card: Card,
    ) -> list[tuple[int, int, int, int, int]]:
    ...
    def _generate_valid_payments(
        ...
        if extra_gold == 2:
            ...
            # Spend no optional gold.
            payments = [minimum_payment]

            for i in range(4, -1, -1):
                if capacities[i] == 0:
                    continue

                # Spend one optional gold on this color.
                payment = minimum_gold.copy()
                payment[i] += 1
                payments.append(tuple(payment))

                # Spend the second on a different color.
                ...
                # Spend both optional gold on the same color.
                if capacities[i] >= 2:
                    payment[i] += 1
                    payments.append(tuple(payment))
