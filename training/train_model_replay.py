...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Checkpoint to load model weights from. By default, "
            "the optimizer state is also restored."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "When used with --resume, load the model weights but "
            "start with a fresh AdamW optimizer instead of restoring "
            "the checkpoint optimizer state."
        ),
    )
    ...
    if args.resume is not None:
        ...
        # Load M0 / previous checkpoint weights either way.
        model.load_state_dict(
            checkpoint["model_state_dict"]
        )

        # Continue epoch numbering from the loaded checkpoint.
        start_epoch = checkpoint["epoch"] + 1

        if args.reset_optimizer:
            # Keep the freshly-created AdamW optimizer above.
            # This preserves the model's learned weights while
            # discarding AdamW's first/second-moment history.
            print(
                f"Loaded model weights from epoch "
                f"{checkpoint['epoch']}"
            )
            print(
                "Optimizer state RESET: using a fresh AdamW optimizer."
            )
        else:
            if "optimizer_state_dict" not in checkpoint:
                raise KeyError(
                    "Checkpoint does not contain optimizer_state_dict. "
                    "Use --reset-optimizer to load weights with a fresh optimizer."
                )
            ...
            print(
                f"Resuming model + optimizer from epoch "
                f"{checkpoint['epoch']}"
            )
